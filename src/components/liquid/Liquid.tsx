...
export default function YourLiquid() {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        const res = await fetch(
          `http://localhost:5000/api/crypto/Usersellerorders/${user.email}`
        );
        if (!res.ok)
              ...
              {displayedOrders.length > 0 ? (
                displayedOrders.map((order) => {
                  ...
                  if (
                    ...
                    const marketPerUnit =
                      marketPrice[order.cryptoType][currencyCode].PRICE;
                    const diffPercent =
                      ((pricePerUnit - marketPerUnit) / marketPerUnit) * 100;
                  ...
                  return (
                    <Link
                      ...
                      className="contents"
                    >
                      <TableRow className="cursor-pointer hover:bg-gray-100 dark:hover:bg-white/5 transition-colors">
                        <TableCell className="px-5 py-4 text-start text-base">
                          <small className="me-1 text-gray-900 dark:text-white">
                            pay
                          </small>
                          <Badge
                            size="sm"
