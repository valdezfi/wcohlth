...
export default function PublicSellerOrdersTable({ fullname }: Props) {
  ...
  // Fetch seller orders by fullname
  useEffect(() => {
    if (!fullname) return;
    ...
    const fetchOrders = async () => {
      try {
        const res = await fetch(
          `http://localhost:5000/api/crypto/sellerordersbyfullname/${encodeURIComponent(fullname)}`
        );
        const data = await res.json();
        if (data.success) {
    ...
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
      <div className="w-full overflow-x-auto">
        <Table className="text-[16px] leading-relaxed">
          <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
            <TableRow>
              <TableCell isHeader className="px-5 py-3 text-[14px] font-semibold text-start text-gray-600 dark:text-gray-300" />
              <TableCell isHeader className="px-5 py-3 text-[14px] font-semibold text-start text-gray-600 dark:text-gray-300">Pay</TableCell>
              <TableCell isHeader className="px-5 py-3 text-[14px] font-semibold text-center text-gray-600 dark:text-gray-300">Receive</TableCell>
            </TableRow>
          </TableHeader>
            ...
            {displayedOrders.length > 0 ? (
              displayedOrders.map((order) => {
                ...
                if (
                  ...
                  differenceBadge = (
                    <Badge size="sm" color={isHigher ? 'error' : 'success'} className="ms-2">
                      {isHigher ? '+' : ''}
                ...
                return (
                  <TableRow key={order.id} className="cursor-pointer hover:bg-gray-100 dark:hover:bg-white/5 transition-colors">
                    <TableCell className="px-5 py-4 text-start">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col text-base">
                          <span className="font-semibold text-gray-900 dark:text-white">{order.sellerFullName}</span>
                          <span className="text-gray-600 dark:text-gray-400">👍 {order.sellerPositiveCount}% rating</span>
                          <span className="text-gray-600 dark:text-gray-400">{order.country}</span>
                          <span className="text-gray-400 text-sm">
                            {order.lastActive
                              ? `Last seen ${formatDistanceToNow(new Date(order.lastActive), { addSuffix: true })}`
                              : 'Last seen: N/A'}
                          </span>
                        </div>
                      </div>
                    ...
                    <TableCell className="px-5 py-4 text-start text-base">
                      <div>
                        ...
                        <Badge size="sm" color="info">{order.paymentMethod}</Badge>
                        <div className="text-gray-900 dark:text-white">
                          ${order.price} <small className="text-sm">💰 {order.currency}</small> {differenceBadge}
                        </div>
                      </div>
                    </TableCell>
      ...
      {visibleCount < sellerOrders.length && (
        <div className="text-center p-4">
          <Button onClick={() => setVisibleCount((prev) => prev + 5)} size="sm" variant="primary">
            Load More
          </Button>
        </div>
      )}
